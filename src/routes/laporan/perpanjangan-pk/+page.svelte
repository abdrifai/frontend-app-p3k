  ...
  const fetchData = async (page = 1) => {
    ...
    try {
      const params = new URLSearchParams({ 
        page, 
        limit: meta.limit,
        status: "SELESAI" // Hardcoded for report
      });
...
<div class="max-w-7xl mx-auto py-6 sm:py-8 px-4 sm:px-6 lg:px-8 space-y-6">
  <div class="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
    <div>
      ...
      <p class="mt-1 text-sm text-slate-500">Daftar perpanjangan kontrak yang telah selesai diproses.</p>
    </div>
  </div>

  <!-- Filters -->
  <div class="card p-4">
    <form on:submit|preventDefault={() => { meta.page = 1; fetchData(); }} class="flex flex-wrap gap-3">
      ...
      <table class="min-w-full divide-y divide-slate-200">
        <thead>
          <tr class="bg-slate-50/80">
            ...
            <th class="px-6 py-4 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Pegawai</th>
            <th class="px-6 py-4 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Periode Kontrak</th>
            <th class="px-6 py-4 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Masa Kerja</th>
            <th class="px-6 py-4 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider text-center">Dokumen</th>
            <th class="px-6 py-4 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider text-right">Aksi</th>
          </tr>
        </thead>
          ...
          {:else}
            {#each records as rec, i}
              <tr class="hover:bg-slate-50/50 transition-colors">
                <td class="px-6 py-4 text-sm text-slate-400 font-mono">{(meta.page - 1) * meta.limit + i + 1}</td>
                <td class="px-6 py-4">
