<script>
  ...
  const statTabs = [
    ...
    { id: "jabatan", label: "Jenis Jabatan", color: "amber" },
  ...
  const fetchStatistics = async () => {
    statsLoading = true;
    try {
      const result = await apiRequest("/api/v1/p3k-csv-import/statistics", "GET");
      if (result.success) {
        stats = result.data;
      } else {
...
<svelte:head>
  <title>Statistik Data P3K Import — App P3K</title>
</svelte:head>
  ...
  <div class="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
    <div>
      <h1 class="text-2xl font-bold text-slate-800">
        Statistik Data P3K Import
      </h1>
      <p class="mt-1 text-sm text-slate-500">
        Ringkasan data pegawai PPPK hasil draf / import dari SIASN.
      </p>
    </div>
    <button
      ...
      <div class="relative">
        ...
        {:else}
          <p class="text-3xl font-extrabold text-slate-800">
            {stats?.totalPegawai?.toLocaleString("id-ID") ?? 0}
          </p>
        {/if}
      </div>
    </div>
      ...
      <div class="relative">
        ...
        {:else}
          <p class="text-3xl font-extrabold text-indigo-600">
            {stats?.genderCount?.laki?.toLocaleString("id-ID") ?? 0}
          </p>
          {#if stats?.totalPegawai}
            <p class="text-xs text-slate-400 mt-1">
              {((stats.genderCount.laki / stats.totalPegawai) * 100).toFixed(
                1,
              )}% dari total
            </p>
          {/if}
      ...
      <div class="relative">
        ...
        {:else}
          <p class="text-3xl font-extrabold text-rose-500">
            {stats?.genderCount?.perempuan?.toLocaleString("id-ID") ?? 0}
          </p>
          {#if stats?.totalPegawai}
            <p class="text-xs text-slate-400 mt-1">
              {(
                (stats.genderCount.perempuan / stats.totalPegawai) *
                100
            ...
            <div class="flex-1">
              <div class="w-full h-4 rounded-full bg-rose-200 overflow-hidden">
                <div
                  ...
                  style="width: {(stats.genderCount.laki / stats.totalPegawai) *
                    100}%"
                ></div>
              </div>
              <div class="flex justify-between mt-1.5">
                <span class="text-xs font-bold text-indigo-600"
                  >{(
                    (stats.genderCount.laki / stats.totalPegawai) *
                    100
                  ).toFixed(0)}% L</span
                >
      ...
      <div class="p-5 sm:p-6">
        <div class="flex items-center justify-between mb-5">
          ...
          {#if !statsLoading && activeStatData.length > 0}
            ...
            >
              {activeStatData.length} kategori
            </span>
          {/if}
        </div>
        ...
        {:else}
          <div class="space-y-3">
            {#each activeStatData as item, idx}
              {@const maxCount = getMaxCount(activeStatData)}
              {@const pct = maxCount > 0 ? (item.count / maxCount) * 100 : 0}
              {@const totalPct = stats?.totalPegawai
                ? ((item.count / stats.totalPegawai) * 100).toFixed(1)
                : 0}
              <div class="group">
            ...
            <div class="space-y-2">
              {#each tabData.slice(0, 5) as item}
                <div class="flex items-center justify-between">
                  <span
                    class="text-xs text-slate-600 truncate max-w-[70%]"
