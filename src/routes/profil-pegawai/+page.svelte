                ...
                {#each profile.riwayatKontrak as k}
                  <tr class="hover:bg-slate-50/80 transition-colors">
                    ...
                    <td class="py-3.5 px-4 text-right whitespace-nowrap">
                      {#if k.arsipKontrak?.fileUrl || k.fileUrl}
                        {@const fileUrl = k.arsipKontrak?.fileUrl || k.fileUrl}
                        {#if isPdf(fileUrl)}
                          <button
                            type="button"
                            on:click={() => openPdfModal(fileUrl, `Dokumen SK Kontrak Ke-${k.kontrakKe}`, k.nomorKontrak ? `Nomor Kontrak: ${k.nomorKontrak}` : '')}
                            class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-blue-50 hover:bg-blue-100 text-blue-700 rounded-lg text-xs font-semibold transition-colors border border-blue-200 shadow-xs cursor-pointer"
                            title="Klik untuk melihat preview dokumen SK"
                          >
                        ...
                        {:else}
                          <a
                            href={`${API_BASE_URL}${fileUrl}`}
                            target="_blank"
                            class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-slate-50 hover:bg-slate-100 text-slate-700 rounded-lg text-xs font-semibold transition-colors border border-slate-200 shadow-xs"
            ...
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
              ...
              <div class="flex flex-wrap items-center gap-2">
                {#if profile.arsipSkCpns?.fileUrl}
                  {#if isPdf(profile.arsipSkCpns.fileUrl)}
                    <button
                      type="button"
                      on:click={() => openPdfModal(profile.arsipSkCpns.fileUrl, 'SK Pengangkatan Pertama', profile.nomorSkCpns ? `Nomor: ${profile.nomorSkCpns}` : '')}
                      class="px-2.5 py-1 bg-blue-600 hover:bg-blue-700 text-white rounded-md text-xs font-semibold flex items-center gap-1 transition-colors shadow-xs cursor-pointer"
                    >
                      <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
          ...
          <div class="p-4 sm:p-5 rounded-xl bg-rose-50 border border-rose-200 space-y-2.5 min-w-0">
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
              ...
              {#if profile.arsipSkPensiun.fileUrl}
                {#if isPdf(profile.arsipSkPensiun.fileUrl)}
                  <button
                    type="button"
                    on:click={() => openPdfModal(profile.arsipSkPensiun.fileUrl, 'SK Pensiun Pegawai', profile.arsipSkPensiun.nomorSk ? `Nomor SK: ${profile.arsipSkPensiun.nomorSk}` : '')}
                    class="px-3 py-1 bg-white hover:bg-rose-100 text-rose-700 rounded-lg text-xs font-semibold border border-rose-200 flex items-center gap-1 transition-colors self-start sm:self-auto cursor-pointer"
                  >
                    <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                    </svg>
                    Preview SK Pensiun (.pdf)
                  </button>
                {:else}
                  <a
...
{#if showPdfModal}
  <div
    class="fixed inset-0 z-50 overflow-y-auto bg-slate-900/70 backdrop-blur-xs flex items-center justify-center p-2 sm:p-4 md:p-6 transition-all duration-200"
    role="dialog"
    aria-modal="true"
  >
    <div class="bg-white w-full max-w-5xl h-[92vh] max-h-[900px] rounded-2xl shadow-2xl flex flex-col border border-slate-200 overflow-hidden">
      <!-- Modal Header -->
      <div class="px-4 sm:px-6 py-3.5 bg-slate-900 text-white flex items-center justify-between shrink-0 shadow-md">
        <div class="flex items-center gap-3 min-w-0">
          <div class="w-8 h-8 rounded-lg bg-blue-500/20 text-blue-400 flex items-center justify-center shrink-0 border border-blue-400/30">
            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
            </svg>
          </div>
          <div class="min-w-0">
            <h3 class="text-sm sm:text-base font-bold text-white truncate">{pdfModalTitle}</h3>
            {#if pdfModalSubtitle}
              <p class="text-[11px] sm:text-xs text-slate-300 truncate font-mono">{pdfModalSubtitle}</p>
            {/if}
          </div>
        </div>
        ...
        <div class="flex items-center gap-2 shrink-0">
          <a
            ...
            target="_blank"
            download
            class="px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-200 rounded-lg text-xs font-semibold flex items-center gap-1.5 transition-colors border border-slate-700"
          ...
          <a
            href={pdfModalUrl}
            target="_blank"
            class="px-3 py-1.5 bg-blue-600 hover:bg-blue-500 text-white rounded-lg text-xs font-semibold flex items-center gap-1.5 transition-colors shadow-sm"
          ...
          <button
            ...
            class="p-1.5 text-slate-400 hover:text-white hover:bg-slate-800 rounded-lg transition-colors ml-1"
            title="Tutup Preview"
          >
            <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
